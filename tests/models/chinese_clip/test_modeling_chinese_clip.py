...
from transformers import ChineseCLIPConfig, is_torch_available
...
class ChineseCLIPModelTester:
    ...
    def create_and_check_for_causal_lm(
        ...
        model = ChineseCLIPLMHeadModel(config=config)
        model.to(torch_device)
        model.eval()
    ...
    def create_and_check_for_masked_lm(
        ...
        model = ChineseCLIPForMaskedLM(config=config)
        model.to(torch_device)
        model.eval()
        result = model(input_ids, attention_mask=input_mask, token_type_ids=token_type_ids, labels=token_labels)
        self.parent.assertEqual(result.logits.shape, (self.batch_size, self.seq_length, self.vocab_size))
    ...
    def create_and_check_decoder_model_past_large_inputs(
        ...
        config.is_decoder = True
        config.add_cross_attention = True
        model = ChineseCLIPLMHeadModel(config=config).to(torch_device).eval()
    ...
    def create_and_check_for_next_sequence_prediction(
        ...
        model = ChineseCLIPForNextSentencePrediction(config=config)
        model.to(torch_device)
        model.eval()
        result = model(
            input_ids,
    ...
    def create_and_check_for_pretraining(
        ...
        model = ChineseCLIPForPreTraining(config=config)
        model.to(torch_device)
        model.eval()
        result = model(
            input_ids,
    ...
    def create_and_check_for_question_answering(
        ...
        model = ChineseCLIPForQuestionAnswering(config=config)
        model.to(torch_device)
        model.eval()
        result = model(
            input_ids,
    ...
    def create_and_check_for_sequence_classification(
        ...
        model = ChineseCLIPForSequenceClassification(config)
        model.to(torch_device)
        model.eval()
    ...
    def create_and_check_for_token_classification(
        ...
        config.num_labels = self.num_labels
        model = ChineseCLIPForTokenClassification(config=config)
        model.to(torch_device)
        model.eval()
        result = model(input_ids, attention_mask=input_mask, token_type_ids=token_type_ids, labels=token_labels)
    ...
    def create_and_check_for_multiple_choice(
        ...
        model = ChineseCLIPForMultipleChoice(config=config)
        model.to(torch_device)
        model.eval()
...
@require_torch
class ChineseCLIPModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    ...
    all_generative_model_classes = (ChineseCLIPLMHeadModel,) if is_torch_available() else ()
    fx_compatible = False
    ...
    def setUp(self):
        ...
        self.config_tester = ConfigTester(self, config_class=ChineseCLIPConfig, hidden_size=37)
    ...
    def test_torchscript_device_change(self):
        ...
        for model_class in self.all_model_classes:
            ...
            # ChineseCLIPForMultipleChoice behaves incorrectly in JIT environments.
            if model_class == ChineseCLIPForMultipleChoice:
                return
